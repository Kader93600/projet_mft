...
import { PACK_METADATA, fmtEuros, type PackSlug } from "@/lib/packs";
...
interface Props {
  /** Prix actifs depuis la DB (server-fetched). */
  prices: PriceEntry[];
  /** Slug de formation pré-sélectionné (depuis ?formation=...). */
 ...
 *
 * - Sélecteur de formation horizontal (pills avec accent couleur formation)
 * - 3 cards de packs asymétriques (Initial petit, Medium moyen, Premium en vedette)
 * - Si formation = Capacité ≤ 3,5 t : seul Initial est affiché + message
 *
 * Tone : Premium est mis en avant émotionnellement (sessions présentielles
 * + accompagnement humain). Le copywriting reflète l'investissement
 * personnel, pas juste les features.
 */
export function PricingSection({ prices, defaultFormationSlug }: Props) {
  // Toutes les formations qui ont au moins un prix actif
  ...
  const initialSlug =
    defaultFormationSlug && formationsWithPrices.some((f) => f.slug === defaultFormationSlug)
  ...
  // Prix de la formation sélectionnée
    ...
    >
      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          <span className="italic font-normal text-signal-400">pas le nôtre</span>.
        </h2>
              ...
              {formationsWithPrices.map((f) => {
                ...
                return (
                  <button
                    ...
                    className={
                      "inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold transition-all duration-200 motion-reduce:transition-none whitespace-nowrap " +
                      (isActive
                        ? "bg-white text-night-900 shadow-glow-signal"
                        : "bg-white/[0.06] text-white/70 border border-white/10 hover:bg-white/[0.10] hover:text-white")
                    }
                    style={
                      isActive
                        ? { boxShadow: `0 0 0 1px ${f.accent}, 0 8px 24px -8px ${f.accent}60` }
                        : undefined
                    }
                  >
                    <span
      ...
      {/* Cards des packs — asymétriques : Initial à gauche compact,
          Medium au centre, Premium à droite (mis en avant émotionnel) */}
        ...
        {isCapaciteOnly ? (
          ...
          <div className="grid lg:grid-cols-12 gap-5 md:gap-6 items-stretch">
            ...
            <MediumCard
              ...
              formationCode={selectedFormation?.code}
            />
            {/* Premium — colonne 4 (mais visuellement plus dominant) */}
            <PremiumCard
...
function InitialCard({
  ...
  return (
    <article
      className="lg:col-span-4 relative flex flex-col rounded-3xl bg-white/[0.03] border border-white/10 p-7 md:p-8 backdrop-blur-sm"
    ...
    >
      ...
      <div className="mt-7 flex items-baseline gap-2">
        <span className="font-display text-4xl font-semibold text-white tracking-tight">
          {priceCents != null ? fmtEuros(priceCents) : "—"}
        </span>
        {compareAtCents != null && (
...
// =====================================================================
// MEDIUM — Pack intermédiaire, ton "équilibre"
// =====================================================================
function MediumCard({
  priceCents,
  compareAtCents,
  ...
  return (
    <article
      className="lg:col-span-4 relative flex flex-col rounded-3xl bg-white/[0.05] border border-signal-500/20 p-7 md:p-8 backdrop-blur-sm"
    ...
    >
      ...
      <p className="mt-2 text-[14.5px] text-white/65 leading-relaxed">
        Tout l'Initial, plus une <strong className="text-white">messagerie
        privée</strong> avec votre formateur dédié, joignable du début à la
...
// =====================================================================
// PREMIUM — Vedette de la page, ton émotionnel et exécutif
// =====================================================================
function PremiumCard({
  ...
  return (
    <article
      className="lg:col-span-4 relative flex flex-col rounded-3xl p-7 md:p-9 overflow-hidden"
      aria-labelledby="pack-premium-title"
      style={{
        background:
          "linear-gradient(165deg, rgba(161, 98, 7, 0.18) 0%, rgba(11, 15, 36, 0.95) 50%, rgba(11, 15, 36, 1) 100%)",
    ...
    >
      ...
      <div className="text-[11px] font-bold uppercase tracking-[0.18em] mb-2"
        style={{ color: "#FCD34D" }}
      >
        Pack Premium
      </div>
      ...
      >
        ...
        <br />
        <span className="italic font-normal" style={{ color: "#FCD34D" }}>
      ...
      <div className="mt-7 flex items-baseline gap-2">
        <span
          className="font-display text-5xl font-semibold tracking-tight text-white"
      ...
      <ul className="mt-7 space-y-3 text-[15px] text-white/90">
        ...
        <FeatureLine highlight goldAccent>
          <span className="inline-flex items-center gap-1.5">
            <GraduationCap className="h-3.5 w-3.5" style={{ color: "#FCD34D" }} />
...
// =====================================================================
// CAPACITÉ ≤ 3,5 T — Layout spécifique (Initial uniquement)
// =====================================================================
function CapaciteOnlyLayout({
  ...
  return (
    <div className="grid lg:grid-cols-12 gap-6 items-stretch">
      ...
      <div className="lg:col-span-5 rounded-3xl bg-white/[0.03] border border-white/10 p-7 md:p-8 flex flex-col justify-center">
        ...
        <p className="mt-3 text-[14.5px] text-white/65 leading-relaxed">
          La <strong className="text-white">Capacité ≤ 3,5 t</strong> est une
          formation courte et standardisée (105 h théorie). L'examen est
          national, sur QCM uniquement : un suivi rapproché ou des sessions
          présentielles n'apportent pas de valeur ajoutée significative.
        </p>
        <p className="mt-3 text-[14.5px] text-white/65 leading-relaxed">
...
function CTAButton({
  ...
  const styles =
    variant === "gold"
      ? "bg-amber-500 text-night-900 hover:bg-amber-400 shadow-glow-signal"
      : variant === "secondary"
        ? "bg-signal-500 text-night-900 hover:bg-signal-400"
        : "bg-white/[0.08] text-white border border-white/15 hover:bg-white/[0.14]";
  return (
    <Link
      href={`/inscription?formation=${formationSlug}&pack=${pack}`}
      className={
        "group inline-flex items-center justify-center gap-2 w-full px-5 py-3 rounded-xl text-sm font-semibold transition-all duration-200 motion-reduce:transition-none " +
        styles
      }
    >
      {label}
      <ArrowRight className="h-4 w-4 group-hover:translate-x-0.5 motion-reduce:transition-none transition" />
    </Link>
  );
}
